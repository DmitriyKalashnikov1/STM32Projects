...
#include "Drivers/NRF24_conf.h"
#include "Drivers/NRF24_reg_addresses.h"
...
void nrf24_open_tx_pipe(uint8_t *addr){
	nrf24_w_reg(TX_ADDR, addr, 1);//nrf24_w_reg(TX_ADDR, addr, 5);
}
...
uint8_t nrf24_transmit(uint8_t *data, uint8_t size){
	...
	csn_low();
	HAL_StatusTypeDef st = HAL_SPI_Transmit(&hspiX, &cmd, 1, spi_w_timeout);
	HAL_SPI_Transmit(&hspiX, data, size, spi_w_timeout);
	csn_high();
	...
	if (st == HAL_TIMEOUT){
		...
		printf("Unknown error while transmit \r\n");
	}
...
//	}
	...
	nrf24_w_reg(0x00, 0x08, 1);
	nrf24_w_reg(0x01, 0x3f, 1);
	nrf24_w_reg(0x02, 0x03, 1);
	nrf24_w_reg(0x03, 0x03, 1);
	nrf24_w_reg(0x04, 0x03, 1);
	nrf24_w_reg(0x05, 0x02, 1);
	nrf24_w_reg(0x06, 0x0f, 1);
	nrf24_w_reg(0x07, 0x0e, 1);
	nrf24_w_reg(0x08, 0x00, 1);
	nrf24_w_reg(0x09, 0x00, 1);
		uint8_t pipeAddrVar[6];
		pipeAddrVar[4]=0xE7; pipeAddrVar[3]=0xE7; pipeAddrVar[2]=0xE7; pipeAddrVar[1]=0xE7; pipeAddrVar[0]=0xE7;
	nrf24_w_reg(0x0A, pipeAddrVar, 5);
		pipeAddrVar[4]=0xC2; pipeAddrVar[3]=0xC2; pipeAddrVar[2]=0xC2; pipeAddrVar[1]=0xC2; pipeAddrVar[0]=0xC2;
	nrf24_w_reg(0x0B, pipeAddrVar, 5);
	nrf24_w_reg(0x0C, 0xC3, 1);
	nrf24_w_reg(0x0D, 0xC4, 1);
	nrf24_w_reg(0x0E, 0xC5, 1);
	nrf24_w_reg(0x0F, 0xC6, 1);
		pipeAddrVar[4]=0xE7; pipeAddrVar[3]=0xE7; pipeAddrVar[2]=0xE7; pipeAddrVar[1]=0xE7; pipeAddrVar[0]=0xE7;
	nrf24_w_reg(0x10, pipeAddrVar, 5);
	nrf24_w_reg(0x11, 0, 1);
	nrf24_w_reg(0x12, 0, 1);
	nrf24_w_reg(0x13, 0, 1);
	nrf24_w_reg(0x14, 0, 1);
	nrf24_w_reg(0x15, 0, 1);
	nrf24_w_reg(0x16, 0, 1);
	uint8_t cmdRxBuf[2];
	csn_low();
	cmdRxBuf[0] = 0x50;
	cmdRxBuf[1] = 0x73;
	nrf24_w_reg(0x1a, cmdRxBuf, 2);
	csn_high();
	nrf24_w_reg(0x1c, 0, 1);
	nrf24_w_reg(0x1d, 0, 1);
