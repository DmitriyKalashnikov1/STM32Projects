	...
	//	printf("copter V int: %li\n\r", message.V_out_int);
		...
		if (message.id == copter_id){
			...
			if (currentIndex <= FILTER_SIZE){
				receivedCurrents[currentIndex] = message.data.I_out_int;
				receivedVoltages[currentIndex] = message.data.V_out_int;
				currentIndex += 1;
			}else {
				currentIndex = 0;
				Filter();
...
//		NRF24_stopListening();
		...
		int dataIsHere = NRF24_available(); // после обработки прерывания -- сбрасываем флаги
